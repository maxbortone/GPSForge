...
def gaussian(loc=1.0, scale=1e-2, dtype=jnp.float32):
...
def input_kernel_init(dtype=np.float32):
...
def variational_kernel_init(loc=1.0, scale=1e-2, dtype=jnp.float32):
    def init(key, shape, dtype=dtype):
        n_in, n_out = shape
        N = int((n_out/n_in)*2)
        L = int(n_out/N)
