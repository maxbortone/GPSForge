...
def renormalize_log_psi(n_spins : Array, hilbert : HomogeneousHilbert, index : int) -> Array:
    """
    Renormalizes the log-amplitude to conserve the number of spin-up and down electrons

    Args:
        n_spins : number of spin-up and down electrons up to index (batch, 4)
        hilbert : Hilbert space from which configurations are sample
        index : site index
    """
...
def train():
    # Parser
    parser = create_parser('Train an Ansatz on a 1D hydrogen chain using VMC')
