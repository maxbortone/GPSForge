...
if isinstance(args.J2, float):
    df = df.loc[df['J2'] == args.J2]
else:
    args.J2 = list(np.unique(df['J2'].values))
...
# Print table
df = df.sort_values(by=['N', 'ansatz', 'J2', 'samples'], ascending=[True, True, True, True])
print(df[['N', 'ansatz', 'J2', 'samples', 'energy', 'error', 'rel_error', 'uuid']].to_markdown(index=False, tablefmt='github'))

# Plot
unique_N = np.unique(df['N'].values)
if isinstance(args.J2, float):
    f = sns.relplot(
        data=df, kind='line',
        x='N', y='rel_error',
    ...
    f.axes[0,0].set_xticks(unique_N)
    f.axes[0,0].set_xticklabels(unique_N)
elif isinstance(args.J2, list):
    f = sns.relplot(
        data=df, kind='line',
        x='N', y='rel_error',
        col='J2', col_order=sorted(args.J2),
        hue='samples', style='ansatz',
        markers=True, dashes=False,
        palette='tab10', markersize=10)
    f.set(xscale='log', yscale='log')
